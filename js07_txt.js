...
document.getElementById('getFile').onchange = function () {
      ...
      // Verify that the user to select a text file
      try {
            let isText = userFile.type.startsWith('text')
            if (!isText) {
                  throw userFile.name + " is not a text file";
            }
            // Read the coontents of the selected file
      ...
      function wordCloud(sourceText) {
            //Conver the source text to lowercase
            // and romove leading and trailing whitespaces
            sourceText = sourceText.toLowerCase();
            sourceText = sourceText.trim();
            //Leave only alhpabet cheacters and whitespace in the text
            ...
            // containing a word and its duplicates
            ...
            for (let i = 1; i <words.length; i++) {
                  if (words[i] === words[i-1]) {
                        //Increases the duplicate count by 1
